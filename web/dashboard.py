...
def _cors(response):
    """Add CORS headers — allowlist only."""
    _allowed = {"http://localhost:5555", "https://backyardhummers.com"}
    origin = request.headers.get("Origin", "")
    if origin in _allowed:
        response.headers["Access-Control-Allow-Origin"] = origin
        response.headers["Vary"] = "Origin"
    else:
        response.headers["Access-Control-Allow-Origin"] = "https://backyardhummers.com"
...
_SPAM_URL_RE = _re.compile(r'https?://|www\.|\.com/|\.net/|\.org/', _re.IGNORECASE)
_SPAM_WORDS = ['fuck', 'shit', 'ass', 'damn', 'bitch', 'dick', 'cunt', 'piss',
               'asshole', 'bastard', 'stfu', 'wtf', 'slut', 'whore']
_SPAM_WORD_RE = _re.compile(
    r'\b(?:' + '|'.join(_re.escape(w) for w in _SPAM_WORDS) + r')\b', _re.IGNORECASE
)


def _is_spam(text: str) -> bool:
    """Check if text contains URLs or profanity."""
    return bool(_SPAM_URL_RE.search(text) or _SPAM_WORD_RE.search(text))
...
def api_guestbook_post():
    ...
    # Honeypot check
    if data.get("website", ""):
        return _cors(jsonify({"ok": False, "error": "Validation failed."})), 422

    # Timing challenge — reject < 3 seconds
    elapsed = int(data.get("_t", 0) or 0)
    if elapsed < 3000:
        return _cors(jsonify({"ok": False, "error": "Please take your time."})), 422
    ...
    # Content filtering
    if _is_spam(name) or _is_spam(message):
        return _cors(jsonify({"ok": False, "error": "Your message could not be posted."})), 422
...
@app.route("/api/guestbook/<int:entry_id>", methods=["DELETE", "OPTIONS"])
def api_guestbook_delete(entry_id):
    if request.method == "OPTIONS":
        return _cors(Response("", status=204))
    if not _monitor or not _monitor.sightings_db:
        return _cors(jsonify({"ok": False, "error": "Service unavailable"})), 503
    deleted = _monitor.sightings_db.delete_guestbook_entry(entry_id)
    if deleted:
        return _cors(jsonify({"ok": True}))
    return _cors(jsonify({"ok": False, "error": "Entry not found."})), 404
